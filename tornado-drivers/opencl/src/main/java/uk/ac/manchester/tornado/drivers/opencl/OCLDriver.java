...
public final class OCLDriver extends TornadoLogger implements TornadoAcceleratorDriver {
    public static final List<OCLDeviceType> DEVICE_TYPE_LIST_ORDERING = Arrays.asList(OCLDeviceType.CL_DEVICE_TYPE_GPU, OCLDeviceType.CL_DEVICE_TYPE_CPU, OCLDeviceType.CL_DEVICE_TYPE_ACCELERATOR,
    ...
    public OCLDriver(final OptionValues options, final HotSpotJVMCIRuntime vmRuntime, TornadoVMConfig vmConfig) {
        ...
        flatBackends = orderFlattenBackends(DEVICE_TYPE_LIST_ORDERING);
    }
    ...
    /**
     * Orders the flatBackends by device type, with GPUs first, then CPUs, and
     * finally accelerators (e.g., FPGAs). Uses a bubble sort algorithm for
     * simplicity can be improved to use Comparator.
     */
    private OCLBackend[] orderFlattenBackends(List<OCLDeviceType> deviceTypeOrdering) {
        List<OCLBackend> backendList = new ArrayList<>();
        Map<OCLDeviceType, OCLBackend> deviceTypeMap = new HashMap<>();

        // Populate deviceTypeMap with the first instance of each device type
        for (OCLBackend backend : flatBackends) {
            OCLDeviceType deviceType = backend.getDeviceContext().getDevice().getDeviceType();
            deviceTypeMap.putIfAbsent(deviceType, backend);
        }

        // Add backends to backendList in the order specified by deviceTypeOrdering
        for (OCLDeviceType deviceType : deviceTypeOrdering) {
            OCLBackend backend = deviceTypeMap.get(deviceType);
            if (backend != null) {
