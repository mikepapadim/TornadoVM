...
import uk.ac.manchester.tornado.drivers.opencl.graal.lir.PureRegisterComputation;
 ...
 * {@code OCLAssemblerConstants.GLOBAL_MEM_MODIFIER} is the empty string. ptxas
 * therefore lowers these to a <b>generic</b> {@code LD}, which may target shared
 * memory, so it may not be reordered against a shared store. Hand-written OCL
 ...
 * hand-written OCL   L L L L L L L L S S S S S S S S     LDG
 * </pre>
 *
 * The cost of the serialised form is a {@code long_scoreboard} stall of 12.10
 * cycles per issue against 1.45 for the equivalent hand-written OCL, and an issue
 ...
 * address spaces are disjoint in OCL, so no load in the run can observe a store
 * in the run.</li>
 ...
 * </pre>
 *
 * Disable with {@code -Dtornado.opencl.batchGlobalLoads=False}.
 */
public final class OCLGlobalLoadBatching {

    private OCLGlobalLoadBatching() {
    }
     ...
     * consulted: {@code OCLAddressCast} is always built with a {@code null} operand and
     ...
     * operand that is itself a {@link OCLLIROp} expression tree is reported as
    ...
    private static Value plainOrNull(Value v) {
        ...
        return v instanceof OCLLIROp ? null : v;
    }
}
